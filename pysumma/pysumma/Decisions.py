...
class Decisions:
    ...
    def wrt_attribute(self, method, selection):
        org_val = self.get_attribute(method)
        for index, line in enumerate(self.text):
            if line.startswith(method):
                self.text[index] = line.replace(org_val, selection, 1)
        self.edit_save(self.text)

    def edit_save(self, selectlines):
        filepath = self.path + self.filename
    ...
    # 			return line.split()[1]
    #
    # def wrt_attribute(self, attr, value):
    # 	org_val = self.get_attribute(attr)
    # 	for index, line in enumerate(self.text):
    # 		if line.startswith(attr):
    # 			self.text[index] = line.replace(org_val, value, 1)
    # 	self.EditSave(self.text)
    # 	# return value
    #
    ...
    # #
    # # @ Methods.setter
    # # def Methods(self, new_method):
    # # 	self.wrt_attribute(identifier, new_method)
    # soilCatTbl = property(fget=get_attribute, fset=wrt_attribute)
    # @property
    # def soilCatTbl(self):
    # 	return self.get_attribute('soilCatTbl')
    #
    # @ soilCatTbl.setter
    # def soilCatTbl(self, new_val):
    # 	self.wrt_attribute('soilCatTbl', new_val)
    #
    # @property
    # def vegeParTbl(self):
    # 	return self.get_attribute('vegeParTbl')
    #
    # @ vegeParTbl.setter
    # def vegeParTbl(self, new_val):
    # 	self.wrt_attribute('vegeParTbl', new_val)
